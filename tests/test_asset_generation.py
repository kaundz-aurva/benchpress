...
class AssetGenerationTests(unittest.TestCase):
    ...
    def test_hammerdb_tcl_renders_tprocc_settings_and_markers(self) -> None:
        ...
        self.assertIn("diset connection mssqls_tcp true", tcl)
        self.assertIn("diset connection mssqls_authentication sql", tcl)
        self.assertIn("diset tpcc mssqls_dbase {tpcc}", tcl)
        self.assertNotIn("diset connection mssqls_database", tcl)
        self.assertIn("diset tpcc mssqls_rampup $warmup_minutes", tcl)
        self.assertIn("diset tpcc mssqls_duration $measured_minutes", tcl)
        self.assertNotIn("runtimer", tcl)
    ...
    def test_metrics_scripts_render_logman_and_artifact_markers(self) -> None:
        ...
        self.assertIn("Stop-BenchpressCollectorIfPresent", start_script)
        self.assertIn("logman stop $CollectorName", stop_script)
        self.assertIn("Data Collector Set is not running", stop_script)
